...
struct node *node_create(const char *date)
{
...
void node_print(WINDOW *win, const struct node *node, int depth, int *sy, int *sx)
{	
	mvwprintw(win,*sy, *sx, "%s", node->date);
	(*sy)++;
	
	for (size_t i = 0; i < node->count; i++)
		node_print(win, node->children[i], depth + 1, sy, sx);
}	
...
{
	double cur_jd = swe_julday(cdata->utc_year, cdata->utc_mon, cdata->utc_mday, cdata->utc_hour, SE_GREG_CAL);
	double next_jd = 0.0;
	...
	char date[32];
	char root_date[32];
	snprintf(root_date, 32, "%d, %d, %d", cdata->year, cdata->mon, cdata->mday);
	struct node *root = node_create(root_date);
	double l1[2] = {0};
	double l2[2] = {0};
	l1[0] = cur_jd;
	
	for (int i = 0; (next_jd - cur_jd) < 120 * year; ++i)
	{
		if (i == 1)
			l1[1] = next_jd;
		...
		struct node *child = node_create(date);
		add_child(root, child);
	}
	int sy = 10, sx = 5;
	node_print(win, root, 0, &sy, &sx);
	memcpy(tmp, cdata, sizeof(*cdata));
	struct node *l2_root = node_create(root_date);
	sign = (int)(pxx->dfor[LONG] / 30)+1;
	next_jd = 0.0;
	while(l1[1] > next_jd)
	{
		tmp->mday += (int)(pl_period[sign] * month);
		if (sign++ >= 12)
			sign = 1;
		...
		snprintf(date, 32, "%s: %d.%d.%d", ui->sym.zo_sym[sign], tmp->year, tmp->mon, tmp->mday);
		struct node *child = node_create(date);
		add_child(l2_root, child);
	}
	sy = 10;
	sx += 18;
	node_print(win, l2_root, 0, &sy, &sx);
	getch();
	node_free(root);
	node_free(l2_root);
	free(tmp);
